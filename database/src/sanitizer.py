...
class Sanitizer:
    def __init__(self):
        ...
        self.columns_to_keep = [
            "id", "c_cimulateTags", "c_colorName", "c_fabricTechnology", "c_fill", "c_fit", 
            "c_fsProductDescriptionShort", "c_fsProductName", "c_gender", "c_lengthDescription",
            "modelImageUrl", "otherProductImageUrl"
        ]
    ...
    def run(self):
        ...
        try:
            ...
            df.rename(columns=lambda col: col[2:] if col.startswith("c_") else col, inplace=True)
    ...
    def generate_embedding_tags(self, row):
        ...
        text_fields = [str(row[col]).strip() for col in row.index if col not in self.exclude_from_embedding and pd.notna(row[col])]
